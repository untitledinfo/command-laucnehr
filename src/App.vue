...
onMounted(() => {
  store.loadAll();
  // Subscribed once for the app's lifetime (not tied to any one view) so the
  // Console keeps receiving output even after navigating away from Home
  // right after launching.
  window.launcherApi.launcher.onOutput((line) => store.appendConsole(line));
  window.launcherApi.launcher.onExit(() => {
