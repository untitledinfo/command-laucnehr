...
export class MinecraftInstaller {
    ...
    for (const lib of libraries) {
      ...
      if (artifact?.url) {
        ...
        if (url && relPath) {
          ...
          // Modern versions (1.19+) no longer ship natives via the legacy
          // "classifiers" map below - LWJGL's per-OS natives are just a
          // normal artifact library whose maven coordinate carries a
          // "natives-<os>" classifier (e.g. "org.lwjgl:lwjgl:3.3.3:natives-windows").
          // Such libraries are gated behind an OS "rules" block that
          // isAllowed() already evaluated above, so any one reached here is
          // guaranteed to match the current platform - extract it.
          if (this.isNativesArtifact(lib.name, relPath)) {
            this.status('Extracting natives...');
  ...
  /** True if a library's maven coordinate/path identifies it as a natives-only jar (e.g. LWJGL 1.19+ style). */
  private isNativesArtifact(name: string | undefined, relPath: string | undefined): boolean {
    if (name && /:natives-/.test(name)) return true;
    if (relPath && /-natives-(windows|linux|macos|osx)(-[a-z0-9_]+)?\.jar$/i.test(relPath)) return true;
    return false;
  }
