...
export class MinecraftLauncher {
  ...
  private collectLibraryPaths(mcDir: string, versionJson: any, out: string[]) {
    ...
    for (const lib of libraries) {
      ...
      if (relPath) {
        // Natives-classifier jars (e.g. "org.lwjgl:lwjgl:3.3.3:natives-windows")
        // contain no classes, only extracted .dll/.so/.dylib files - they don't
        // belong on the classpath, only in the natives directory.
        if (/:natives-/.test(lib.name ?? '') || /-natives-[a-z0-9_-]+\.jar$/i.test(relPath)) continue;
