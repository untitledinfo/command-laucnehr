...
export class AccountManager {
  ...
  addOffline(username: string): Account {
    ...
    // Offline UUIDs are deterministic (derived from the username), so adding
    // the same name twice would otherwise create two accounts sharing one
    // UUID. Treat it as re-adding the same account instead.
